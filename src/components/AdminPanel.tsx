...
import WhiteLabelAdmin from "./WhiteLabelAdmin";
...
export default function AdminPanel() {
  ...
  const [abaAtiva, setAbaAtiva] = useState<"usuarios" | "whitelabel">("usuarios");
  ...
  return (
    <div>
      <Cabecalho
        ...
        desc="Gerencie usuários, white label e configurações da sua organização."
      />
                    ...
                    <li key={u.id} className="group flex items-center gap-3 border-b border-sand/60 px-4 py-3 transition last:border-b-0 hover:bg-paper">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="flex items-center gap-2 text-[13px] font-bold text-ink">
                          {u.nome}
                          {u.papel === "admin" && <span className="rounded-sm bg-pine px-1.5 py-0.5 text-[8.5px] font-extrabold text-lime uppercase">Admin</span>}
                        </p>
                        <p className="text-[11px] text-ink-faint">{u.email} {u.cargo && `· ${u.cargo}`}</p>
                      </div>
